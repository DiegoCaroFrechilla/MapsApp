...
class UserPrefs(private val context: Context) {
    companion object {
        ...
        val STORE_PASS_PERMISSION = stringPreferencesKey("false")
    ...
    val getUserData: Flow<List<String>> = context.dataStore.data.map { prefs ->
        listOf(
            ...
            prefs[STORE_PASS_PERMISSION] ?: ""
        )
    }

    suspend fun saveUserData(username: String, userpass: String, userpermission: String) {
        context.dataStore.edit { prefs ->
            prefs[STORE_USERNAME] = username
            prefs[STORE_USERPASS] = userpass
            prefs[STORE_PASS_PERMISSION] = userpermission
        }
    }
}